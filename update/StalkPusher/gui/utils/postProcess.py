...
# 
#     return (truemaxind, np.array(bad_ind))
def weightAdjLoads(x, y, a, m, g):
    '''
    Returns numpy array with each element equal to:
    sqrt((|x|-sin(a)*m*g)^2 + (|y|-cos(a)*m*g)^2)
    for each x in loadsX, corresponding y in loadsY and corresponding a in angles.
    loadsX, loadsY and angles are numpy arrays.
    m (mass of assembly) and g gravity are scalars (float or int)
    angles are in radians.
    loadsX and loadsY and g can be in any units.  
    '''
    return np.sqrt((abs(x) - np.sin(a)*m*g)**2 + (abs(y) - np.cos(a)*m*g)**2)

# def weightAdjLoads(loadsX, loadsY, angles, m, g):
#     '''
#     Returns numpy array with each element equal to:
#     ((|x|-sin(a)*m*g)^2 + (|y|-cos(a)*m*g)^2)
#     for each x in loadsX, corresponding y in loadsY and corresponding a in angles.
#     loadsX, loadsY and angles are numpy arrays.
#     m (mass of assembly) and g gravity are scalars (float or int)
#     angles are in radians.
#     loadsX and loadsY and g can be in any units.  
#     '''
#     adjust = lambda x, y, a: math.sqrt((abs(x) - math.sin(a)*m*g)**2 + (abs(y) - math.cos(a)*m*g)**2)
