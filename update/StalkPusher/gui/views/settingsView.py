...
#             {'label': 'GPS', 'funct': self.toPendingSetting},
            ...
            {'label': 'LCA\nWEIGHT', 'funct': self.setLCAWeight},
        )
...
#             self.stnBtns.append(self.SettingBtn(self.app, {'x': col, 'y': row}, self.stnBtnDefs[i],i == self.stnBtnFocusNum))
    ...
    def setLCAWeight(self):
        print('go to LCA WEIGHT setting')
        self.app.setView(kbv.KeyboardView(self.app, self,
                      d.NUM,
                      'Load cell assembly weight',
                      d.LCA_WEIGHT,
                      self.app.getSetting(d.LCA_WEIGHT),
                      suffix = 'g'
                      ))
...
#         from views import heightSettingView as hv
        ...
        self.app.setView(kbv.KeyboardView(self.app, self,
                      ...
                      'Test height',
                      d.TEST_HEIGHT,
                      self.app.getSetting(d.TEST_HEIGHT),
                      suffix = 'cm'
                      ))
...
#         self.app.setView(hv.HeightView(self.app, self))
    ...
    def toPlotSetting(self):
        ...
        self.app.setView(kbv.KeyboardView(self.app, self,
                              ...
                              'Plot number',
                              d.TEST_PLOT,
                              self.app.getSetting(d.TEST_PLOT),
                              prefix = '#'
                              ))
...
#         self.app.setView(psv.PlotSettingView(self.app, self))
    ...
    def keyboardReturn(self, key, value, status = 1):
        #as of now, there hould not be an if else
        if key == d.OPERATOR:
            self.app.saveSetting(d.OPERATOR, value)
        elif key == d.TEST_PLOT:
            try:
                intVal = int(value)
                ...
                self.pushMsg(msg.Message(self.app, self, self.disp,
                                            ...
                                            btnDefs = (
                                                {'label': 'RETRY', 'id': 'yesBtn', 'funct': (self.popMsg, self.toPlotSetting)},
            ...
            try:
                ...
                self.pushMsg(msg.Message(self.app, self, self.disp,
                                            ...
                                            'Please input a valid integer.',
                                            btnDefs = (
                                                {'label': 'RETRY', 'id': 'yesBtn', 'funct': (self.popMsg, self.toHeightSetting)},
                                                {'label': 'CANCEL', 'id': 'yesBtn', 'funct': self.popMsg},
                                                {},
                                                {}
                                            )
        ...
        elif key == d.LCA_WEIGHT:
            try:
                intVal = int(value)
                self.app.saveSetting(d.LCA_WEIGHT, intVal)
            except:
                self.pushMsg(msg.Message(self.app, self, self.disp,
                                            'Invalid weight value.',
                                            'Please input a valid integer for the load cell assembly weight in grams.',
                                            btnDefs = (
                                                {'label': 'RETRY', 'id': 'yesBtn', 'funct': (self.popMsg, self.setLCAWeight)},
                                                {'label': 'CANCEL', 'id': 'yesBtn', 'funct': self.popMsg},
                                                {},
                                                {}
                                            )
